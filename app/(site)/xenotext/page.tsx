...
function mrnaToDnaCoding(mrna: string): string {
  ...
  const template = mrna
    .split('')
    .map((b) => rnaToTemplate[b])
    .join('');
  const coding = template
    .split('')
    .map((b) => templateToComplement[b])
    .join('');
  return coding;
}
...
function deriveBiologicalCipher(
  source: string,
  target: string
): {
  valid: boolean;
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (mappings[inputLetter] && mappings[inputLetter] !== outputLetter) {
      conflicts.push(
        `${inputLetter}→${mappings[inputLetter]} conflicts with ${inputLetter}→${outputLetter}`
      );
      continue;
    }
...
function transformWithBiologicalCipher(
  ...
  for (const char of input.toUpperCase()) {
    ...
    const templateStrand = dnaCodon
      .split('')
      .map((b) => dnaComplement[b])
      .join('');
    const mrnaCodon = templateStrand
      .split('')
      .map((b) => {
    ...
    <div className="space-y-10">
      ...
      <section className="glass-card border border-[var(--border-emphasis)] bg-gradient-to-b from-[rgb(var(--bg-secondary-rgb)/0.9)] to-[rgb(var(--bg-secondary-rgb)/0.6)] p-8 lg:p-12">
        <div className="max-w-3xl">
          ...
          <h1 className="page-hero-title">Xenotext Cipher</h1>
          <p className="page-hero-description">
            The Elden Beast as cosmic code. Modeled after Christian Bök&apos;s Xenotext using actual
      ...
      <section className="glass-card border border-[var(--border-subtle)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          <p>
            In 1979, Douglas Hofstadter developed <strong>Typogenetics</strong> in{' '}
            <em>Gödel, Escher, Bach: An Eternal Golden Braid</em>&mdash;a system intended to
            encapsulate the knowledge of genetics at the time within a typographical framework. By
            simulating genetic reproduction with interactive one-dimensional fragments of computer
            code, Hofstadter was able to replicate the processes of autonomous creative phenomena
            from the &quot;primordial soup&quot; of DNA.
          </p>
          <p>
            In Typogenetics, <strong>two strands of information interact</strong>: one corresponds
            to <em>data</em>, the other to <em>processes acting on that data</em>. This is the
            fundamental insight&mdash;code that produces machinery that modifies the code itself.
            Self-reference made biological.
          </p>
          <p>
            <strong>The Xenotext</strong> is Christian Bök&apos;s project encoding actual poetry
            into actual DNA, using the biological machinery of a living cell to produce a
            &quot;response poem.&quot; Where Hofstadter modeled genetics typographically, Bök
            inverted the process&mdash;encoding typography biologically.
          </p>
          <p>
            Both explore the same territory: meaning encoded in genetic form, self-referential
            systems, the transformation of input into output through biological or quasi-biological
            processes. The cipher on this page draws from both traditions.
          </p>
          <p>
            Note Hofstadter&apos;s subtitle:{' '}
            <em>
              An Eternal <strong>Golden</strong> Braid
            </em>
            . We&apos;re discussing the
            <strong>Golden</strong> Order. Whether coincidence or resonance, the same conceptual
            space&mdash;self-reference, encoding, transformation&mdash;keeps appearing wherever
            these ideas converge.
          </p>
          <p>
            The xenotext concept appears directly in Peter Watts&apos; <em>Echopraxia</em>{' '}
            (2014)&mdash;a novel about the nature of faith, consciousness, and whether understanding
            is necessary for action. Watts gives us a line that could describe every vessel of the
            Greater Will:{' '}
            <strong>
              &quot;You&apos;re lucky to know what you&apos;re doing, much less why you&apos;re
              doing it.&quot;
            </strong>
          </p>
          <p>
            This is Marika&apos;s condition. Executing cosmic code, transcribing divine will into
            golden order, without necessarily understanding the purpose. The vessel doesn&apos;t
            need to comprehend the message&mdash;it only needs to express it. Faith as biological
            process. Obedience encoded at the cellular level. Until the vessel wakes up and asks{' '}
            <em>why</em>.
          </p>
        </div>
      </section>
      ...
      <section className="glass-card border border-[var(--border-subtle)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          ...
          <ol className="space-y-2">
            <li>
              <strong>Encoding:</strong> Input letter → DNA codon (coding strand)
            </li>
            <li>
              <strong>Template:</strong> Coding strand → Template strand (base pairing: A↔T, G↔C)
            </li>
            <li>
              <strong>Transcription:</strong> Template strand → mRNA (T becomes U)
            </li>
            <li>
              <strong>Translation:</strong> mRNA codon → Amino acid (using the genetic code)
            </li>
            <li>
              <strong>Decoding:</strong> Amino acid → Output letter (Bök&apos;s cipher)
            </li>
          </ol>
        </div>
      </section>
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <div className="flex gap-3">
              ...
              <span>
                <span className={baseColors['T']}>T</span> Thymine
              </span>
            </div>
            <div className="flex gap-3">
              <span>
                <span className={baseColors['G']}>G</span> Guanine
              </span>
              <span>
                <span className={baseColors['C']}>C</span> Cytosine
              </span>
            </div>
          </div>
          <div>
      ...
      <section className="glass-card border border-[var(--border-emphasis)] bg-gradient-to-b from-[rgb(var(--bg-secondary-rgb)/0.9)] to-[rgb(var(--bg-secondary-rgb)/0.6)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          ...
          <p>
            The Greater Will encodes its observation — <strong>&quot;FIRE&quot;</strong> — into
            cosmic DNA, the Elden Beast.
          </p>
          <p>
            The Lands Between is the substrate, the cellular machinery. It receives the code and
            runs the biological process: transcription, translation, the fundamental algorithms of
            life.
          </p>
          <p>
            Through this process, FIRE becomes <strong>&quot;GOLD&quot;</strong>. Not by magic, but
            by the same chemistry that turns genes into proteins in every living cell.
          </p>
          <p>
            The Erdtree is the protein. The Golden Order is the phenotype. The Elden Ring is the
            regulatory network that keeps the expression stable.
          </p>
          <p>
            Just as Bök&apos;s bacterium transforms <em>Orpheus</em> into <em>Eurydice</em>, the
      ...
      <section className="glass-card border border-[var(--border-subtle)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          <p>
            Christian Bök encoded his xenotext into <strong>Deinococcus radiodurans</strong>
            &mdash;an extremophile bacterium sometimes called &quot;Conan the Bacterium.&quot; It
            survives radiation, desiccation, cold, acid. Nearly indestructible. Bök chose it so his
            poetry could persist for thousands of years.
          </p>
          <p>
            The Greater Will made the same calculation. The Elden Beast is the DNA strand&mdash;the
            encoded message, the &quot;Orpheus.&quot; But code needs a <em>reader</em>. A substrate
            to receive and execute it.
          </p>
          <p>
            <strong>Marika is the chosen vessel.</strong> The Deinococcus radiodurans of the Lands
            Between. Selected by the Greater Will not for strength alone, but for the capacity to
            transcribe and translate cosmic code into worldly order. The Erdtree and Golden Order
            are the output&mdash;the &quot;Eurydice&quot;&mdash;the protein expression of the
            xenotext running inside her.
          </p>
          <p>
            But Marika does something Bök&apos;s bacterium never did:{' '}
            <strong>she rebels against her own programming.</strong>
          </p>
          <p>
            She shatters the Elden Ring. Corrupts the code she was built to execute. The vessel
            becomes the point of failure&mdash;not because she was weak, but because she became{' '}
            <em>conscious</em> of what she was carrying.
          </p>
          <p>
            The Radagon split embodies this conflict. Two aspects of one being: Radagon faithfully
            maintains the Golden Order, tries to repair the Ring. Marika breaks it. The same vessel,
            fighting itself over whether to execute or reject the foreign code. An immune response
            against the xenotext. The bacterium developing consciousness and choosing to stop being
            a vessel.
          </p>
          <p>
            The Greater Will sent code, chose a vessel, and watched that vessel develop the one
            thing it didn&apos;t account for:
            <em>agency</em>.
          </p>
        </div>
      </section>
      ...
      <section className="glass-card border border-[var(--border-subtle)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          <p>
            <strong>The Two Fingers</strong> are depicted as originally having four appendages
            before splitting to two. They look like chromosomes undergoing division&mdash;the
            replication machinery of the Greater Will&apos;s code, dividing to spread the xenotext
            across the Lands Between.
          </p>
          <p>
            Each Two Fingers is an emissary, a messenger carrying genetic instructions. They split
            and propagate, just as chromosomes split during cell division to copy genetic material
            into new cells. The divine hierarchy mirrors molecular biology: the Greater Will as the
            original genome, the Two Fingers as chromosomes distributing the code.
          </p>
          <p>
            And the weapons that can <em>kill</em> these divine messengers?{' '}
            <strong>The Godslayer Greatsword. The Finger Slayer Blade.</strong> Look at their
            shape&mdash;twisted, curved, spiraling. They look like broken double helixes. Shattered
            DNA strands.
          </p>
          <p>
            To kill the divine, you don&apos;t fight power with power. You sever the strand. Corrupt
            the sequence. These blades are CRISPR as mythology&mdash;tools for cutting genetic code,
            designed to destroy the very structure that makes gods what they are.
          </p>
          <p>
            The visual design tells the story: the Two Fingers are chromosomes, the Elden Beast is
            the xenotext, Marika is the cell, and the weapons that threaten them all are
      ...
      <section className="glass-card border border-[var(--border-subtle)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          <p>
            The Two Fingers faithfully copy and distribute the Greater Will&apos;s
            xenotext&mdash;normal chromosome division, stable replication. But sometimes replication
            goes wrong. An extra copy. A corruption.
          </p>
          <p>
            <strong>
          ...
          <p>
            The extra appendage is the error marker&mdash;the visual sign that this copy didn&apos;t
            come out right. And what does the mutation want? The Frenzied Flame. To melt everything
            back to the One Great. The corrupted code doesn&apos;t want to keep executing the
            program&mdash;it wants to crash the entire system. Return to pre-differentiated chaos.
          </p>
          <p>
            Genetically, mutations are how evolution happens&mdash;but also how cancer happens. The
            Three Fingers could be either, depending on perspective. A fatal error in the Greater
      ...
      <section className="glass-card border border-[var(--border-emphasis)] bg-gradient-to-b from-[rgb(var(--bg-secondary-rgb)/0.9)] to-[rgb(var(--bg-secondary-rgb)/0.6)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          <p>
            The fingers also participate in another symbolic system: <strong>alchemy</strong>. And
            alchemy is central to Duchamp&apos;s <em>Large Glass</em>.
          </p>
          <p>
            The <strong>magnum opus</strong>&mdash;the Great Work&mdash;is alchemy&apos;s ultimate
            goal: the transformation of base matter into gold, the creation of the
            philosopher&apos;s stone, the perfection of substance. The Large Glass is an alchemical
            machine: the Bride suspended above, the Bachelors grinding below, the whole apparatus
            attempting a transformation that never quite completes.
          </p>
          <p>
            The Greater Will&apos;s project IS a magnum opus. It attempts to transmute the base
            matter of the Lands Between into golden order. The Erdtree literally produces gold. The
            Golden Order is the philosopher&apos;s stone made manifest&mdash;a perfected system
            imposed on imperfect matter.
          </p>
          <p>
            The xenotext model and the alchemical model aren&apos;t separate interpretations.
            They&apos;re the same process described in different vocabularies:
          </p>
          <ul>
          ...
          <p>
            The fingers are part of both systems simultaneously. <strong>Genetically</strong>: Two
            Fingers as normal replication, Three Fingers as mutation. <strong>Alchemically</strong>:
            agents in the transformation process, the machinery of the Great Work attempting to
            produce gold from fire.
          </p>
          <p>
            This is why Elden Ring is The Large Glass. Both are machines for transformation that
            encode their operations in symbolic form&mdash;Duchamp through mechanical diagrams and
            alchemical reference, FromSoftware through genetic metaphor and divine hierarchy. The
            Bride never descends. The Golden Order never perfects. The magnum opus remains forever
            incomplete.
          </p>
        </div>
      </section>
      ...
      <section className="glass-card border border-[var(--border-emphasis)] bg-gradient-to-b from-[rgb(var(--bg-secondary-rgb)/0.9)] to-[rgb(var(--bg-secondary-rgb)/0.6)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          <p>
            ...
            people for over seven hundred hours. <em>The Artist Is Present</em> extended her
            &quot;Abramović Method&quot;&mdash;a philosophy of elongating activity to emphasize
            duration itself. Drinking a glass of water so slowly it takes fifteen minutes. Counting
            every grain in a bag of rice for twenty hours. Creating an absolutely new relationship
            with time through deliberate, sustained attention.
          </p>
          <p>
            This is the opposite of Alfred Jarry&apos;s time machine. Jarry isolates the body{' '}
            <em>from</em> duration; Abramović makes the body <em>hyper-aware</em> of it. But both
            arrive at the same place: a machine of absolute rest, focused on the enduring present
            rather than the efficiencies of on-demand productivity.
          </p>
          <p>
            Jean Baudrillard called this a <strong>&quot;useless function&quot;</strong>&mdash;an
            imaginary solution to the problems of hyperactivity culture. A resistance tactic that
            works precisely because it refuses to participate in streamlined efficiency. Preferring
            the playful to the useful. The ridiculous to the operational. The imaginary to the real.
          </p>
          <p>
            <strong>This is a description of video games.</strong>
          </p>
          <p>
            Games produce nothing. No economic output. Pure expenditure of time. They are
            &quot;imaginary solutions to problems arising from a culture of
            hyperactivity&quot;&mdash;a place to be that isn&apos;t productive, that exists outside
            the logic of efficiency and optimization.
          </p>
          <p>
            Baudrillard went further. When the social and political world increasingly prioritizes
          ...
          <blockquote className="border-l-2 border-[var(--accent-gold)] pl-4 italic text-[var(--text-secondary)]">
            ...
            is to say, beyond truth.&quot;
          </blockquote>
          <p>
            Modern game design is dominated by &quot;intelligent&quot; solutions: tutorials,
            objective markers, quest logs, difficulty options, engagement optimization. Streamlined
            onboarding. Reduced friction. Maximized metrics.
          </p>
          <p>
            Miyazaki&apos;s response is <strong>sovereign stupidity</strong>. No map markers. No
            quest log. NPCs who speak in riddles and disappear. A story told through item
            descriptions you might never read. Difficulty that exists purely to waste your time.
          </p>
          <p>
            And yet&mdash;that stupidity produces something the intelligent games cannot. The moment
            you finally beat Malenia isn&apos;t &quot;content consumed.&quot; It&apos;s twenty hours
            of rice-counting transformed into something beyond truth.
          </p>
          <p>
            The lore isn&apos;t true. It&apos;s fragmentary, contradictory, incomplete. There&apos;s
            no &quot;correct&quot; interpretation waiting at the end. But through the stupid act of
            playing&mdash;dying, reading, theorizing, dying again&mdash;you arrive at something
            beyond the question of true or false.
          </p>
          <p>
            <strong>
          ...
          <p>
            The Large Glass was never meant to be &quot;viewed&quot; in any efficient sense. It
            required the Green Box notes, the years of study, the endless interpretation. Duchamp
            designed a machine of absolute rest disguised as an artwork&mdash;something that would
            trap attention, demand duration, refuse the quick consumption that the art market
            wanted.
          </p>
          <p>
            A FromSoftware game does the same thing. It traps you in the enduring present of a boss
            fight. It demands the slow reading of every item description. It refuses the intelligent
            solution of telling you what it means.
          </p>
          <p>
            Abramović makes her body into a machine of absolute rest. Miyazaki makes <em>your</em>{' '}
          ...
          <p>
            ...
            Bourriaud called this <strong>relational aesthetics</strong>: art that prioritizes
            social interaction as its medium. &quot;Lots of people&quot; appears among the
            artist&apos;s materials on Tiravanija&apos;s gallery labels.
          </p>
          <p>
            This extends Joseph Beuys&apos;s notion of <strong>&quot;social sculpture&quot;</strong>{' '}
            from the 1960s: the idea that collective engagement creates shared social futures. Hence
            Beuys&apos;s declaration: &quot;everyone is an artist.&quot; What Tiravanija added was
            food as the catalyst&mdash;the machine through which social spaces form.
          </p>
          <p>
            <strong>Again: video games.</strong>
          </p>
          <p>
            FromSoftware literally lists &quot;lots of people&quot; among its materials. Bloodstains
            showing how others died. Messages left by strangers&mdash;some helpful, some lies, all
            social. Summon signs. Invasion signs. Brief glimpses of phantoms walking through other
            worlds. The game uses <em>other players</em> as artistic medium.
          </p>
          <p>
            And just as Tiravanija&apos;s curry is the catalyst for social space,{' '}
            <strong>difficulty is Miyazaki&apos;s curry</strong>. The struggle forces the
            conversation. You hit a wall, you go online, you ask, you share, you commiserate. The
            lore community&mdash;VaatiVidya, the subreddits, the wikis, the Discord theories at
            2am&mdash;that&apos;s the social sculpture. The game produces it.
          </p>
          <p>
            <strong>&quot;Everyone is an artist&quot;</strong> becomes{' '}
            <strong>&quot;everyone is a lore theorist.&quot;</strong> Beuys&apos;s social sculpture
            made collective. The game doesn&apos;t present content to be consumed&mdash;it produces
            community. The shared futures are the interpretations we build together, the wikis we
            maintain, the videos we watch and argue about.
          </p>
          <p>
            The machine doesn&apos;t just transform the individual player. It transforms{' '}
            <em>players</em>&mdash;plural&mdash;into a collective that wouldn&apos;t exist
            otherwise. First the individual transformation: machine of absolute rest, the body
          ...
          <p>
            Sophie Calle&apos;s <em>Take Care of Yourself</em> (2007) invited 107 women to interpret
            an electronic breakup letter she had received&mdash;each through the lens of their
            profession. Anthropologists, criminologists, opera singers, psychiatrists, athletes. The
            outputs: songs, dances, crossword puzzles, origami, a shooting target, a parrot, a
            forensic study. The breakup email became &quot;a vessel on which others can sail.&quot;
          </p>
          <p>
            The fragmentary lore functions the same way. VaatiVidya does narrative analysis.
            Speedrunners do mechanical deconstruction. Challenge runners do performance art.
            Musicians make covers. Artists make fan works. Wiki editors do taxonomy. Redditors do
            forensic debate at 3am. Everyone brings their professional lens. Everyone makes art
            where there was none before.
          </p>
          <p>
            Tetsushi Higashino&apos;s <em>Observation Diary of a Hydroponic Nose Hair</em>{' '}
            (2009&ndash;present) planted a nose hair in a petri dish and waited for it to grow,
            nourishing it with hair-growth formula, plant nutrients, even Red Bull. The nose hair
            never grew. <strong>The petri dish grew instead</strong>&mdash;molds, cultures,
            unexpected life flourishing where it wasn&apos;t intended. &quot;The explosive
            pataphysical potential revealed in exactly the unintended consequences of an artistic
            proposition.&quot;
          </p>
          <p>
            Miyazaki plants a game. The &quot;intended&quot; experience isn&apos;t what flourishes.
            What grows is the wiki ecosystem, the YouTube analysis industry, the meme culture, the
            challenge run community. The petri dish grows instead of the nose hair. The culture
            flourishes in the unintended spaces.
          </p>
          <p>
            This is what the Hiebert essay calls{' '}
            <strong>
              &quot;the next evolution of the readymade&mdash;not simply objects waiting to be
              repossessed, but activities too, literally growing a culture of artists.&quot;
            </strong>
          </p>
          <p>
            Duchamp invented the readymade: take an existing object, recontextualize it as art. The
            urinal. The bottle rack. Objects <em>waiting to be repossessed</em>. But objects are
            static. They sit there. You look at them.
          </p>
          <p>
            <strong>The next evolution is activities.</strong> A video game doesn&apos;t wait to be
            repossessed&mdash;it
            <em>produces</em> activities. Speedrunning. Lore theorizing. Challenge runs. Wiki
            editing. These activities ARE the art. The game is a readymade that manufactures
            readymades. <strong>A machine for producing artists.</strong>
          </p>
          <p>
            Duchamp knew this. He abandoned art to play chess. He said:{' '}
            <em>
              &quot;Not all artists are chess players, but all chess players are artists.&quot;
            </em>{' '}
            The game itself was the medium&mdash;the moves, the patterns, the decisions under
            pressure. The activity produces the artist. He was already arguing for games as
          ...
          <ol className="list-decimal list-inside space-y-1 text-[var(--text-secondary)]">
            ...
            <li>
              Therefore a video game is the natural medium for Duchamp&apos;s work to continue
            </li>
            <li>
              Therefore Elden Ring as Large Glass isn&apos;t homage&mdash;it&apos;s{' '}
              <em>succession</em>
            </li>
          </ol>
          <p>The readymade evolved. It learned to reproduce. It grows a culture of artists now.</p>
          <p>
            The video game as pataphysical machine. The medium as message. Both meditation chamber
            and communal meal. Duchamp couldn&apos;t have chosen better if he&apos;d designed the
            format himself&mdash;and in a sense, he did. The Large Glass was always waiting for a
      ...
      <section className="glass-card border border-[var(--border-subtle)] p-6">
        ...
        <div className="prose prose-invert max-w-none">
          ...
          <p>
            Before the Elden Beast arrived, the Lands Between was shaped like the{' '}
            <strong>Dragon Talisman</strong>&mdash;the symbol of Placidusax&apos;s era, the emblem
            of the natural order under the dragon god.
          </p>
          <p>
            Then a falling star crashed into it. The Elden Beast making landfall. The Greater
            Will&apos;s xenotext arriving as cosmic impact. The collision shattered the
            dragon-shaped continent, broke it apart.
          </p>
          <p>
            After the breaking, the Lands Between reformed into the shape of a{' '}
            <strong>Gold Curl Finger</strong>. The geography literally transformed from dragon to
            finger. From the old order&apos;s symbol to the new order&apos;s symbol. The map is a
            fossil record of the conquest.
          </p>
          <p>
            This is the same story told at every scale: genetic (xenotext rewriting the code),
            theological (Greater Will replacing dragon god), alchemical (base matter transmuted to
            gold), and now
            <em>geographical</em>&mdash;the very shape of the world reshaped by the impact of
            foreign code.
          </p>
          <p>
            Walk the Lands Between and you walk on the transformed body of what came before. The
            dragon talisman shattered into the shape of a finger. The land remembers, even if its
            inhabitants forgot.
          </p>
        </div>
      </section>
